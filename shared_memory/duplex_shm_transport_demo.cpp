...
#include "duplex_shm_transport.hpp"
#include <atomic>
#include <chrono>
#include <cstdint>
#include <cstring>
...
static constexpr const char* kTransportName = "duplex_shm_transport_demo";
...
int main(int argc, char** argv) {
    ...
    try {
        ...
        if (isA || isA30) {
            ...
            duplex_shm_transport::ShmFixedSlotDuplexTransport::remove(kTransportName);

            std::atomic<uint64_t> received{0};
            duplex_shm_transport::ShmFixedSlotDuplexTransport transport(
                kTransportName,
                slotSize,
                slotCount,
            ...
            std::atomic<uint64_t> received{0};
            auto transport = duplex_shm_transport::ShmFixedSlotDuplexTransport::open(
