...
```cpp
using namespace duplex_shm_transport;
...
```bash
bazel build --config=profile //:duplex_shm_transport_demo
perf stat -d -d ./bazel-bin/duplex_shm_transport_demo --a 5 1024 1024
perf record -F 999 -g -- ./bazel-bin/duplex_shm_transport_demo --b 10
perf report
```
...
```bash
bazel build //:duplex_shm_transport_demo
```

Musl static (if configured):

```bash
bazel build --config=musl //:duplex_shm_transport_demo
```

Run:

```bash
./bazel-bin/duplex_shm_transport_demo --a 10 1024 1024
./bazel-bin/duplex_shm_transport_demo --b 10
```
